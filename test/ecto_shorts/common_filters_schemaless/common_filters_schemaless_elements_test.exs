defmodule EctoShorts.CommonFilters.SchemalessElementsTest do
  ...
  describe ":elements wrapper (schemaless)" do
    ...
    test ":elements works alongside field_types: opt (field_types takes precedence for type routing)" do
      expected = from(p in "posts", where: fragment("? && ?", p.tags, ^["elixir", "ecto"]))
