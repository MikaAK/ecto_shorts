defmodule EctoShorts.CommonQueryTest do
  ...
  describe "lookup_binding_expr/2" do
    test "resolves binding from schema query with no alias" do
      query = from p in Post

      assert %FromExpr{as: nil, source: {"posts", Post}} =
               CommonQuery.lookup_binding_expr(query, nil)
    end

    test "resolves binding from schema query with alias" do
      query = from p in Post, as: :post

      assert %FromExpr{as: :post, source: {"posts", Post}} =
               CommonQuery.lookup_binding_expr(query, nil)
    end
    ...
    test "resolves binding from composed query" do
      ...
      assert %FromExpr{as: nil, source: {"posts", Post}} =
               CommonQuery.lookup_binding_expr(second_query, nil)
    end
    ...
    test "resolves binding from 1-level subquery" do
      ...
      assert %FromExpr{as: nil, source: {"posts", Post}} =
               CommonQuery.lookup_binding_expr(outer_query, nil)
    end
    ...
    test "finds aliased binding by name in subquery" do
      ...
      assert %FromExpr{as: :post, source: {"posts", Post}} =
               CommonQuery.lookup_binding_expr(outer_query, :post)
    end
    ...
    test "finds nested join binding by alias" do
      ...
      assert %JoinExpr{as: :author} = CommonQuery.lookup_binding_expr(query, :author)
    end
    ...
    test "resolves join binding from 1-level subquery" do
      ...
      assert %JoinExpr{as: :author} = CommonQuery.lookup_binding_expr(outer, :author)
    end
    ...
    test "resolves subquery join by alias" do
      ...
      assert %JoinExpr{as: :comments, source: %Ecto.SubQuery{}} =
               CommonQuery.lookup_binding_expr(query, :comments)
    end
    ...
    test "resolves binding expression from nested subquery join where alias is defined in inner query" do
      ...
      assert %FromExpr{as: :comments, source: {"comments", nil}} =
               CommonQuery.lookup_binding_expr(query, :comments)
    end
  end

  describe "lookup_binding_expr_source/2" do
    test "returns source from schema query with no alias" do
      query = from p in Post
      assert {"posts", Post} = CommonQuery.lookup_binding_expr_source(query, nil)
    end

    test "returns source from explicit custom tuple" do
      query = from p in {"custom_source", Post}
      assert {"custom_source", Post} = CommonQuery.lookup_binding_expr_source(query, nil)
    end

    test "returns source from table namequery" do
      query = from p in "posts"
      assert {"posts", nil} = CommonQuery.lookup_binding_expr_source(query, nil)
    end
    ...
    test "resolves source by alias from base query" do
      query = from p in Post, as: :post
      assert {"posts", Post} = CommonQuery.lookup_binding_expr_source(query, :post)
    end
    ...
    test "resolves join source by alias" do
      ...
      assert {nil, User} =
               CommonQuery.lookup_binding_expr_source(query, :author)
    end
    ...
    test "resolves join source after composition" do
      ...
      assert {nil, User} =
               CommonQuery.lookup_binding_expr_source(outer, :author)
    end

    test "resolves schema source from subquery (no alias)" do
      inner = from p in Post
      outer = from p in subquery(inner)
      assert {"posts", Post} = CommonQuery.lookup_binding_expr_source(outer, nil)
    end
    ...
    test "resolves schema source from subquery (with alias, with alias lookup)" do
      inner = from p in Post, as: :post
      outer = from p in subquery(inner), where: p.published_at == true
      assert {"posts", Post} = CommonQuery.lookup_binding_expr_source(outer, :post)
    end
    ...
    test "resolves nil schema from subquery join" do
      subquery = from c in "comments", where: c.approved == true
      query = from p in Post, join: c in subquery(subquery), as: :comments, on: c.post_id == p.id
      assert {"comments", nil} = CommonQuery.lookup_binding_expr_source(query, :comments)
    end
    ...
    test "11" do
      ...
      assert {nil, User} = CommonQuery.lookup_binding_expr_source(query, :author)
    end
    ...
    test "12" do
      ...
      assert {nil, User} = CommonQuery.lookup_binding_expr_source(query, :author)
      assert {nil, User} = CommonQuery.lookup_binding_expr_source(query, :authors)
      assert {nil, Comment} = CommonQuery.lookup_binding_expr_source(query, :comments)
      assert {nil, Post} = CommonQuery.lookup_binding_expr_source(query, :post)
    end
    ...
    test "13" do
      ...
      assert {"comments", Comment} = CommonQuery.lookup_binding_expr_source(query, :comments)
    end
    ...
    test "15" do
      ...
      assert {nil, EctoShorts.Schemas.User} = CommonQuery.lookup_binding_expr_source(query, :user)
    end
  end
end
