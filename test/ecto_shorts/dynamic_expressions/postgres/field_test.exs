defmodule EctoShorts.DynamicExpressions.Postgres.FieldTest do
  ...
  describe "create_dynamic/4 with operator :==" do
    test "with nil" do
      assert_dynamic dynamic([d], is_nil(d.id)),
                     Field.build_dynamic(nil, :id, :==, nil)
    end

    test "with scalar" do
      assert_dynamic dynamic([d], d.id == ^1),
                     Field.build_dynamic(nil, :id, :==, 1)
    end

    test "with list" do
      assert_dynamic dynamic([d], d.id in ^[1, 2, 3]),
  ...
  describe "create_dynamic/4 with operator :!=" do
    test "with nil" do
      assert_dynamic dynamic([d], not is_nil(d.id)),
                     Field.build_dynamic(nil, :id, :!=, nil)
    end
    ...
    test "with list" do
      assert_dynamic dynamic([d], d.id not in ^[1, 2, 3]),
                     Field.build_dynamic(nil, :id, :!=, [1, 2, 3])
    end
