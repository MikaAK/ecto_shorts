defmodule EctoShorts.CommonQuery do
  ...
  def get_binding_expr_source(query, binding_alias) do
    with nil <- lookup_binding_expr_source(query, binding_alias) do
      raise ArgumentError,
            """
  ...
  defp lookup_binding_expr_source(query, binding_alias) do
  ...
  defp lookup_join_expr_source(
    ...
    if has_subquery?(from_expr) do
      ...
      lookup_join_expr_source(join_expr, inner_query)
    else
    ...
    case get_in(joins, [Access.at(binding_position)]) do
      ...
      %{assoc: {^binding_position, ^assoc_key}} = _join_expr ->
        with {_, parent_schema} <-
               ...
               |> lookup_join_expr_source(query) do
          {nil, SchemaHelpers.fetch_schema_assoc_module!(parent_schema, assoc_key)}
        end
      ...
      parent_join_expr ->
        with {_, parent_schema} <- lookup_join_expr_source(parent_join_expr, query) do
          {nil, SchemaHelpers.fetch_schema_assoc_module!(parent_schema, assoc_key)}
        end
    end
  end
  ...
  def get_binding_expr(query_source, binding_alias) do
    query_source
    |> to_query()
    |> get_binding_expr(binding_alias)
  end

  defp lookup_binding_expr(%{from: from_expr, joins: joins} = query, pos, binding_alias)
    ...
    if is_nil(binding_alias) do
      ...
      with nil <- lookup_binding_expr(from_expr, pos, binding_alias) do
        lookup_binding_expr(joins, pos, binding_alias)
      end
    end
  end
  ...
  defp lookup_binding_expr([join_expr | joins], pos, binding_alias) do
    with nil <- lookup_binding_expr(join_expr, pos + 1, binding_alias) do
      lookup_binding_expr(joins, pos + 1, binding_alias)
    end
  end
