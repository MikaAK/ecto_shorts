defmodule EctoShorts.QueryBuilder.Schema do
  ...
  def create_schema_filter({filter_field, val}, schema, query) do
    cond do
        ...
        case schema.__schema__(:type, filter_field) do
          {:array, _} ->
            ComparisonFilter.build_array(
              query,
              schema.__schema__(:field_source, filter_field),
              val
            )
      ...
      true ->
        Logger.debug(
          "[EctoShorts] #{Atom.to_string(filter_field)} is not a field for #{schema.__schema__(:source)} where filter"
        )
  ...
  defp add_relational_alias(query, new_alias) do
    if query.aliases[new_alias] do
      raise ArgumentError,
        message: "already defined #{new_alias} as an alias within query #{inspect(query)}"
    else
