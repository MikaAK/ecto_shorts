...
| 04 | Subquery operands + behavior changes | `from`/`all`/`any`/`exists`/`parent` operands (registered-alias sources, post-build sibling/parent validation); plus D-NULL, D-RAISE (malformed-value & `gt nil`), D-ONE-WAY, D-PROVIDER, single-pass `sort_filter_params` (D2), aggregate→HAVING + auto-GROUP-BY. | D-OPERAND(subquery), D-NULL, D-RAISE, D-ONE-WAY, D-PROVIDER, D2 |
