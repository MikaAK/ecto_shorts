...
| 05 | Wiring + thin adapter + subquery operands + aggregate→HAVING | Route predicates through `TermResolver` → a thin `build_dynamic` adapter (tidying removed from `Postgres`); move `common_field_for` into `TermResolver`; subquery operands (`from`/`all`/`any`/`exists`/`parent`, registered-alias sources, post-build sibling/parent validation); aggregate→HAVING + auto-GROUP-BY; integrate end-to-end; reconcile both suites. | integration, D-OPERAND(subquery), D-CommonExpr-FIELD |
